...
import com.hlpriver.aevmod.container.CChestContainer;
...
public class ChestTileEntity extends BasicLootTileEntity {
    //Number of players opening the chest
    private int playerCount;
    
    // Fixed number of rows for the chest GUI (fixed to 9 for now) 
    private static final int NUM_ROWS = 9;  // 暂时固定为9行
    //Constructor
    ...
    public ChestTileEntity(){
        super(MTileEntities.CHEST_TILE_ENTITY.get());
    }
    //Set GUI name
    @Override
    protected ITextComponent getDefaultName() {
        return new TranslationTextComponent("container."+ PlatinumCore.MOD_ID+".chest");
    }
    //Set GUI size - 9*9 grid (temporarily fixed to 9x9)
    @Override
    protected Container createMenu(int id, PlayerInventory player) {
        // 暂时固定使用9x9的GUI
        return new CChestContainer(id,player,this,9);
    }
    //Set GUI size - fixed to 81 slots (9*9)
    @Override
    public int getSizeInventory() {
        return 9 * NUM_ROWS; // 9列乘以9行 = 81个槽位
    }
